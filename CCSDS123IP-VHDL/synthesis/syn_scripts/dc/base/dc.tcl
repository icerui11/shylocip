...
if {$synopsys_program_name == "dc_shell"}  {
  ...
  # default value and should be changed to a central location for best results.
    ...
    set_app_var alib_library_analysis_path "./results"
...
set REPORTS_DIR "report/${TEST_ID}"
set RESULTS_DIR "results/${TEST_ID}"
...
set OPTIMIZATION_FLOW  "rtm_exp" ;# Specify one flow out of hplp | hc | rtm_exp
#################################################################################
# Search Path Setup
#
...
puts "RM-Info: Finished reading [which ${DCRM_RTL_READ_SCRIPT}]\n"
# OR
...
# Set operating condition on top level
 ...
 set_operating_conditions -analysis_type bc_wc -max WCMIL -min BCMIL
...
#
# redirect ${REPORTS_DIR}/${DCRM_MULTIBIT_COMPONENTS_REPORT} {report_multibit -hierarchy }
#################################################################################
